...
col_subdiv = 40; % Reco. 100. This is how many subdivisions are made in the x dimension to form the grids that will be used to build up the final image.
...
save_name = sprintf('%s focused image Settings %i %i %i %i.png', files(1), tolerance, col_subdiv, row_subdiv, noise_neighbours);
imwrite(uint8(final_image),fullfile(path,save_name),'png');
save_name = sprintf('%s image map Settings %i %i %i %i.png', files(1), tolerance, col_subdiv, row_subdiv, noise_neighbours);
